    ...
    describe('register', function () {
        ...
        it('should create item object', function (done) {
            async.waterfall([
                // ダミーログイン
                function (nextTask) {
                    superagent.get('http://localhost:' + port + '/debug/login').end(function (res) {
                ...
                function (nextTask) {
                    ...
                    superagent.post('http://localhost:' + port + '/items/create').send(data).set('Cookie', cookie).end(function (res) {
                        nextTask(null, res);
                    });
                },
    ...
    describe('get', function () {
        it('should get item object', function (done) {
            ...
            async.waterfall([
                ...
                function (user, nextTask) {
                    superagent.get('http://localhost:' + port + '/items/1').end(function (res) {
        ...
        it('should update item', function (done) {
            async.waterfall([
                ...
                function (nextTask) {
                    ...
                    superagent.post('http://localhost:' + port + '/items/1/edit').send(updated).set('Cookie', cookie).end(function (res) {
                        nextTask(null, res);
                    });
                },
                // 更新処理がDBに反映されているか.
    ...
    describe('like request ', function () {
        it('should reposed 404 status when user like to no exists item', function (done) {
            ...
            async.waterfall([
                ...
                function (nextTask) {
                    superagent.post('http://localhost:' + port + '/items/1/like').set('Cookie', cookie).end(function (err, res) {
                        nextTask(err, res);
                    });
                }
            ], function (err, res) {
        ...
        it('anonymous user should not like', function (done) {
            ...
            async.waterfall([
                ...
                function (nextTask) {
                    // anonymous user
                    superagent.post('http://localhost:' + port + '/items/1/like').end(function (err, res) {
                        nextTask(err, res);
                    });
                }
            ], function (err, res) {
