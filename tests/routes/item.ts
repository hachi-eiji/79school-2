...
describe('item', function () {
  ...
  describe('register', function () {
    ...
    it('should create item object', function (done) {
      async.waterfall([
        ...
        function (res:http.IncomingMessage, nextTask:Function) {
          expect(res.statusCode).to.equal(200);
          nextTask();
        },
        function (nextTask:Function) {
          // APIで作成したアイテムがDBに入っているか
          Item.find({ownerId: 1}).populate('owner').exec(function (err:Error, items:ItemDocument[]) {
            nextTask(null, items);
          });
        }
      ], function (err, items) {
  ...
  describe('get', function () {
    it('should get item object', function (done) {
      ...
      async.waterfall([
        function (nextTask:Function) {
          User.findOne({id: 1}, function (err:Error, user:UserDocument) {
            nextTask(null, user);
          });
        },
        function (user:UserDocument, nextTask:Function) {
          var item = {
            id: '1',
            ownerId: 1,
            owner: user._id,
  ...
  describe('edit', function () {
    ...
    it('should update item', function (done) {
      async.waterfall([
        ...
        // 更新処理がDBに反映されているか.
        function (res:http.IncomingMessage, nextTask:Function) {
          Item.findOne({id: '1'}, function (err:Error, item:ItemDocument) {
            nextTask(err, item);
          });
        }
  ...
  describe('like request ', function () {
      ...
      async.waterfall([
        ...
        function (nextTask:Function) {
          superagent
            ...
            .set('Cookie', cookie)
            .end(function (err:Error, res:superagent.Response) {
              nextTask(err, res);
            });
        }
    ...
    it('should create like on item collection', function (done) {
      ...
      async.waterfall([
        ...
        function (res:superagent.Response, nextTask:Function) {
          expect(res.status).to.equal(200);
